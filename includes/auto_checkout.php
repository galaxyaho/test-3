<?php
/**
 * Auto Checkout System for L.P.S.T Hotel Booking System
 * Works with existing bookings and resources tables
 */
...
class AutoCheckout {
    ...
    public function executeDailyCheckout() {
        try {
            ...
            // Check if it's time for auto checkout (only for automatic runs)
            if (!$isManualRun) {
    ...
    private function getBookingsForCheckout() {
        ...
        // Get all active bookings that are past their checkout time or are from yesterday
        $stmt = $this->pdo->prepare("
            ...
            AND (
                DATE(b.check_in) < ?
                OR (DATE(b.check_in) = ? AND TIME(b.check_in) <= TIME(NOW() - INTERVAL 1 HOUR))
            )
    ...
    public function getCheckoutStats() {
        try {
            ...
            $stmt = $this->pdo->prepare("
                ...
                JOIN payments p ON acl.booking_id = p.booking_id
